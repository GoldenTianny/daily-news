...
역할 규칙
- 아직 가입 안 한 사람은 **이메일로 미리 지정** (구글 로그인하는 순간 자동 적용)
- **마스터**: 관리자·부관리자 지정/해제, 통계 열람. 마스터는 SQL 로만 바꿀 수 있음
- **관리자**: 부관리자 지정/해제, 통계 열람
- **부관리자**: 통계 열람만

조회 기록은 ETF 검색기에서 종목·ETF 상세를 열 때 남습니다. 회원은 계정으로, 비회원은 브라우저 무작위 식별자로 구분됩니다 (같은 화면 5분 내 재조회는 1건).

기능을 추가한 뒤에는 `supabase/001_admin.sql` 을 **다시 한 번 실행**하면 됩니다 (여러 번 실행해도 안전).
