...
| `d1`~`d4` | DOUBLE | 각 수축의 깊이(%), 없으면 NULL |
| `base_days` | USMALLINT | 베이스 고점 이후 경과 거래일 |
...
- 판정: 최근 120거래일 최고 고가를 베이스 시작점으로, 5% 이상 반전만 골라 수축 2~5회 추출. 첫 수축이 가장 깊고 35% 이하. 완성은 마지막 수축 12% 이하이면서 첫 수축의 60% 이하이고 계속 좁아지는 경우, 형성 중은 20%·85%의 완화 기준. 종가가 피벗 대비 −20% ~ +5%
- **거래량 미반영** — 원전 VCP는 각 수축에서 거래 감소를 함께 보지만 거래량 자료가 없어 가격 쪽만 판정
- 규모: 15,453행 · 399거래일 · 544KB. 소비처: `tools/etf/minervini.html` (회원 전용 카드)
