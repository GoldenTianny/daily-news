...
- 동점(거래정지로 가격이 불변이면 점수 1.0으로 다수 동점)은 같은 순위(동점 그룹의 마지막 순위)를 받아 재계산해도 결과가 동일
...
`tools/market/build_vcp.py`가 OHLC와 미너비니 템플릿에서 계산 (ingest_daily 12단계). **월별** 파일 `YYYY-MM.parquet`.

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `date` / `code` / `name` | | 거래일 / 종목코드 / 종목명 |
| `tf` | VARCHAR | 시간축 — `D` 일봉 / `W` 주봉 (같은 날짜에 두 행) |
| `close` / `rs` | | 종가 / RS 등급 |
| `in_tpl` | BOOLEAN | 템플릿 8개 전부 통과(true) / 조건 ⑤만 미달(false) |
| `pivot_px` | DOUBLE | 매수 기준선 = 마지막 수축의 고점 |
| `dist` | DOUBLE | 종가의 피벗 대비 거리(%), 음수면 아직 아래 |
| `n_cont` | UTINYINT | 수축 횟수 (2~5) |
| `d1`~`d5` | DOUBLE | 각 수축의 깊이(%), 없으면 NULL |
| `base_days` | USMALLINT | 베이스 고점 이후 경과 — 일봉은 거래일, 주봉은 주 |
| `tight` | DOUBLE | 마지막 수축 깊이(%) |
| `tier` | VARCHAR | `fit` 완성 / `near` 형성 중 |

- 모집단: 미너비니 템플릿 8개 통과 **또는** 조건 ⑤(종가 > 50일선)만 미달. 베이스 후반에 50일선을 잠시 내주는 종목을 놓치지 않기 위함이며, 추세 구조 조건(②③④)은 반드시 충족
- 시간축: 일봉(창 120거래일 · 베이스 최소 10거래일 · 반전폭 5%)과 주봉(창 52주 · 최소 3주 · 반전폭 6%)을 함께 계산. 주봉 막대는 월~일로 묶고 기준일이 속한 주는 그날까지의 미완성 막대
- 판정: 창 안의 최고 고가를 베이스 시작점으로, 반전폭 이상만 골라 수축 2~5회 추출. 첫 수축이 가장 깊고 35% 이하. 완성은 마지막 수축 12% 이하이면서 첫 수축의 60% 이하이고 계속 좁아지는 경우, 형성 중은 20%·85%의 완화 기준. 종가가 피벗 대비 −20% ~ +5%
- **거래량 미반영** — 원전 VCP는 각 수축에서 거래 감소를 함께 보지만 거래량 자료가 없어 가격 쪽만 판정
- 규모: 37,646행 · 399거래일 · 1.1MB. 소비처: `tools/etf/minervini.html` (회원 전용 카드, 일봉·주봉 전환)
...
- 일일 파일 `ETF_price_concensus_*.xlsx` 스냅샷 시트의 `수정시가`·`수정고가`·`수정저가` 열(기준일 1일치, 2026-09-14~)도 같은 DB에 병합 (ingest_daily 3단계)
- 범위: 2023-12-28 ~ (기준일), 종목 2,651개, 1,649,070행 · 17MB
...
|---|---|---|
| `date` | DATE | 거래일 |
| `code` / `name` | VARCHAR | 종목코드 / 종목명 |

- **거래정지인 (date, code)만 저장** — 행이 없으면 그 날 정상 거래. 원본 전 종목이 '정상' 또는 '거래정지'로 채워져 있어 결측과 구분됨 (가격 DB에 없는 미상장 종목은 애초에 대상 아님)
- 원본에 있는 날짜는 통째로 교체(정지 해제 반영), 없는 날짜는 유지. 내용이 같은 달은 재기록하지 않음
- 용도: 백테스트에서 매매 불가일 제외, 신고가·RS 판정 시 정지 종목 필터 등
  ```sql
  SELECT p.* FROM 'db/market/price/*.parquet' p
  LEFT JOIN 'db/market/halt/*.parquet' h USING (date, code) WHERE h.code IS NULL
  ```
