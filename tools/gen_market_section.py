...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/688a9839-daily_20260824.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/669b387e-20260824.html"
PUB = "archive/2026-08-21.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-08-24.html"      # 출력
BASE = "2026-08-21"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.08.24"              # 발행일 (업데이트 표기)
GRAD = "824"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,913 — 전일 +0.88% 이틀째 상승</strong>입니다. 다만 <strong>코스닥은 802로 -4.63% 급락</strong>해 대형주와 중소형주의 온도차가 컸습니다. <strong>외국인은 -0.46조 순매도</strong>로 돌아섰습니다. 미국은 S&P500 +0.43%·다우 +0.98%로 반등했습니다.',
 '💡 <strong>원/달러 1,388원(-0.14%)</strong> — 1,390원 아래에서 강세 흐름을 이어갔습니다(1개월 -6.37%). 미 국채 바이백 후폭풍에 달러가 약세를 보인 영향입니다. <strong>엔화는 158.95엔</strong>입니다.',
 '💡 <strong>금 4,624.10달러(+3.00%) — 사상 최고 재경신, 처음으로 4,600달러를 넘었습니다</strong>(1개월 +13.58%). <strong>은도 68.03달러로 +6.39% 급등</strong>했고, <strong>두바이유는 97.81달러(+4.50%)</strong>로 100달러에 다가섰습니다. WTI 87.06달러, 브렌트유 94.39달러입니다.',
 '💡 <strong>미국 10년물 4.738%(+4.0bp)</strong> — 다시 4.7%대 중반으로 올라섰습니다. <strong>국고채 3년 3.854%(+4.3bp), 회사채 4.542%(+3.8bp)</strong>로 국내 금리도 동반 상승했습니다.',
 '💡 <strong>고객예탁금 106.58조, 신용잔고 30.87조</strong> 수준입니다. 이번 자료의 자금 항목은 직전 집계치가 그대로 유지돼(미갱신) 증감보다 수준으로 보시는 편이 안전합니다.',
 '💡 <strong>외국인 -0.46조 — 순매도 전환</strong>(1주 누적 -2.22조). <strong>개인도 -0.54조</strong>, 기관 -0.10조로 3주체 모두 소폭 매도 우위였습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 8월 21일 6,913pt로 전일 +0.88%, 이틀째 올랐습니다. 다만 코스닥은 -4.63% 급락해 대형주와 중소형주의 온도차가 컸고, 외국인은 -0.46조 원 순매도로 돌아섰습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 8월 21일 7,674pt로 전일 +0.43% 반등했습니다. 다우 +0.98%, 나스닥 +0.43%입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 66,016pt로 전일 -0.30% 소폭 하락했습니다(1주 -3.93%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.738%(+4.0bp)로 다시 4.7%대 중반으로 올라섰습니다. 한국 국고채 3년도 3.854%(+4.3bp)로 동반 상승했습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,388원(-0.14%)으로 1,390원 아래에서 강세 흐름을 이어갔습니다. 1개월 기준 -6.37%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $87.06으로 +1.43% 상승했습니다. 두바이유가 $97.81(+4.50%)로 100달러에 다가섰고(1개월 +18.57%), 브렌트유는 $94.39입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,624.10으로 +3.00% 급등해 처음으로 4,600달러를 넘어섰습니다(1개월 +13.58%). 은도 $68.03으로 +6.39% 급등했습니다.'),
]
