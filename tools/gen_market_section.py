...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/80d1e164-daily_20260820.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/fc97ae00-20260820.html"
PUB = "archive/2026-08-19.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-08-20.html"      # 출력
BASE = "2026-08-19"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.08.20"              # 발행일 (업데이트 표기)
GRAD = "820"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,471 — 전일 -5.80% 급락</strong>했습니다. 미국 국채금리발 충격에 <strong>외국인이 -3.47조 대규모 순매도로 전환</strong>했고, 개인이 +4.56조로 받아냈습니다. <strong>닛케이도 65,326(-3.16%)로 이틀째 급락</strong>, 반면 미국은 S&P500 +0.31%로 소폭 반등했습니다.',
 '💡 <strong>원/달러 1,413.5원(-0.35%)</strong> — 급락장에도 원화는 오히려 강세입니다(1개월 -5.13%). <strong>엔화도 158.14엔(-0.89%)</strong>으로 안전통화 강세입니다.',
 '💡 <strong>금 4,366.00달러(-0.33%) — 사상 최고권에서 이틀째 소폭 조정</strong>(1개월 +8.80%). 유가는 상승 지속 — WTI 84.94달러(+3.08%), <strong>두바이유 90.29달러로 90달러 돌파</strong>, 브렌트유 91.02달러. 은 63.94달러입니다.',
 '💡 <strong>미국 10년물 4.696%(-1.0bp)</strong> — 재무부의 국채 바이백 확대 방침에 진정됐습니다. <strong>국고채 3년 3.798%(-4.9bp), 회사채 4.497%(-4.5bp)</strong> — 주식 급락 속 채권으로 피난하며 국내 금리는 하락했습니다.',
 '💡 <strong>고객예탁금 104.76조(+4.04조 급증)</strong> — 변동성 장세에 대기자금이 크게 늘었습니다(8/18 집계). 신용잔고 30.67조(+2,342억), 수익증권은 -3.72조 감소입니다.',
 '💡 <strong>외국인 -3.47조 — 나흘 매수 뒤 대규모 순매도 전환</strong>. <strong>개인은 +4.56조 저가 매수</strong>로 맞섰고, 기관은 -1.27조입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 8월 19일 6,471pt로 전일 -5.80% 급락했습니다. 미국 국채금리발 충격에 외국인이 -3.47조 원 대규모 순매도로 돌아섰고, 개인이 +4.56조 원으로 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 8월 19일 7,716pt로 전일 +0.31% 반등했습니다. 나스닥 +0.16%, 다우 +0.20%로 소폭 회복입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,326pt로 전일 -3.16%, 이틀째 급락했습니다(1주 -3.25%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.696%(-1.0bp)로 재무부의 국채 바이백 확대 방침에 진정됐습니다. 한국 국고채 3년은 3.798%(-4.9bp) — 주식 급락 속 안전자산 선호로 하락했습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,413.5원(-0.35%)으로 급락장에도 원화가 오히려 강세입니다. 1개월 기준 -5.13%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $84.94로 +3.08% 상승했습니다. 두바이유가 $90.29로 90달러를 넘어섰고, 브렌트유 $91.02. 1개월 기준 +2.97%입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,366.00으로 -0.33% 내려 사상 최고권에서 이틀째 소폭 조정입니다(1개월 +8.80%). 은은 $63.94입니다.'),
]
