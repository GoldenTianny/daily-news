...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/cda6de80-daily_20260909.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/bc92cf64-20260909.html"
PUB = "archive/2026-09-08.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-09.html"      # 출력
BASE = "2026-09-08"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.09"              # 발행일 (업데이트 표기)
GRAD = "909"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,955 — 전일 -0.58%, 7,000 안착에 실패</strong>했습니다. 장중 2% 넘게 오르다 막판 차익매물에 밀렸습니다. 다만 <strong>외국인 +0.69조 사흘째 순매수</strong>·기관 +0.42조로 수급은 받쳐줬고, 개인이 -2.88조 차익실현이었습니다. <strong>코스닥 812(-1.25%)</strong>, 미국은 다우 -1.07%·S&P500 -0.55%, 닛케이 -0.84%로 글로벌 동반 약세였습니다.',
 '💡 <strong>원/달러 1,346원(-0.26%) — 1,340원대 원화 강세가 이어집니다</strong>(3개월 -13.70%). <strong>엔화는 153.99엔</strong>으로 1주 새 -3.86%의 강세를 보였습니다.',
 '💡 <strong>두바이유 105.39달러(+3.41%) 급등 — 1개월 +30.51%</strong>로 고유가가 심화됐습니다. 브렌트유 97.00달러(+0.75%), 알루미늄 3,310달러(+0.53%)입니다. <strong>WTI(91.48달러)·금(4,429.80달러)·은(66.05달러)</strong> 시세는 이번 자료에서 미갱신이라 수준 참고용입니다.',
 '💡 <strong>미국 10년물 4.769%(-2.3bp) — 이틀째 하락</strong>했습니다. 글로벌 큰손들이 美국채를 팔고 주식을 담는다는 소식 속에서도 금리는 진정 흐름입니다. <strong>국고채 3년 3.901%(+0.1bp)</strong>, 회사채 4.575%(보합)로 국내 금리는 1개월 +15.5bp 오른 뒤 숨 고르기입니다.',
 '💡 <strong>고객예탁금 93.93조(+3,758억)</strong> — 하루 소폭 늘었지만 1주 새 -5.78조 줄었습니다(9/7 집계). <strong>신용잔고 32.76조(-4,021억)</strong>로 하루 감소했으나 1개월 +3.95조 늘어난 상태라, 반등 때 빚투가 15% 급증했다는 기사처럼 레버리지는 유의할 부분입니다. 미수잔고 1.15조(+947억), 수익증권 +7.10조입니다.',
 '💡 <strong>외국인 +0.69조 — 사흘째 순매수</strong>(1주 누적 +1.99조). <strong>개인은 -2.88조</strong>로 1주 새 -11.85조 대규모 차익실현이며, 기관은 +0.42조입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 8일 6,955pt로 전일 -0.58%, 7,000 안착에 실패했습니다. 장중 2% 넘게 오르다 막판 차익매물에 밀렸지만 외국인은 +0.69조 원으로 사흘째 순매수를 이어갔습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 8일 7,676pt로 전일 -0.55% 하락했습니다. 다우 -1.07%, 나스닥 -0.32%로 3대 지수가 동반 약세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,278pt로 전일 -0.84% 하락했습니다(1주 -1.42%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.769%(-2.3bp)로 이틀째 하락했습니다. 한국 국고채 3년은 3.901%(+0.1bp)로 1개월 +15.5bp 오른 뒤 숨 고르기입니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,346원(-0.26%)으로 1,340원대 원화 강세가 이어졌습니다. 3개월 기준 -13.70%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $91.48 수준이며 이번 자료에서 미갱신입니다. 두바이유는 $105.39(+3.41%)로 급등해 1개월 +30.51%, 브렌트유는 $97.00(+0.75%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,429.80 수준으로 이번 자료에서 미갱신입니다(1개월 +8.17%). 은($66.05)도 미갱신입니다.'),
]
