...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/101b521f-daily_20260921.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/258cbeaf-2026-09-21.html"
PUB = "archive/2026-09-18.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-21.html"      # 출력
BASE = "2026-09-18"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.21"              # 발행일 (업데이트 표기)
GRAD = "921"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,894 — 전일 +2.66% 급등</strong>하며 주간 낙폭을 거의 되돌렸습니다(1주 -0.23%). <strong>외국인이 +0.38조로 이레 만에 매수 전환</strong>했고 기관도 +1.53조 가세했습니다. 다만 <strong>개인은 -3.58조</strong> 대규모 차익실현이었습니다. <strong>닛케이 +1.38%</strong>도 강세, 미국은 나스닥 +0.39%·다우 -0.18%로 혼조였습니다.',
 '💡 <strong>원/달러 1,389원(+0.83%) — 사흘째 올라 1주 +3.36%</strong>입니다. 원화 약세가 빨라지며 1개월 기준으로도 -1.73%까지 강세 폭이 좁혀졌습니다. <strong>엔화는 156.89엔(+0.58%)</strong>으로 1주 +2.18% 약세입니다.',
 '💡 <strong>두바이유 118.40달러(-7.50%) 급락</strong>했지만 1개월 +31.13%로 고유가 기조는 유지됩니다. WTI 100.30달러(-2.08%)는 100달러를 지켰고, 브렌트유 103.87달러(-1.85%)입니다. <strong>금 4,385.90달러(+0.91%)·은 66.56달러(+3.53%)</strong>는 이틀째 올랐습니다.',
 '💡 <strong>미국 10년물 4.996%(+4.9bp) — 다시 5% 문턱</strong>입니다(1개월 +29.0bp). <strong>SOFR가 3.85%로 +23bp 급등</strong>해 단기 자금시장 경색 신호도 나타났습니다. 반면 <strong>국고채 3년 4.035%(-2.8bp)</strong>, 회사채 4.706%(-2.0bp)로 국내 금리는 내렸습니다.',
 '💡 <strong>고객예탁금 97.49조(-2.08조)</strong> — 1주 새 -10.17조 급감했습니다(9/17 집계). 금리에 민감해진 자금이 정기예금으로 옮겨가는 흐름(사흘 만에 7조 유입)과 맞물립니다. <strong>신용잔고 32.66조(-29억)</strong>는 보합, 미수잔고 1.01조(-286억)입니다.',
 '💡 <strong>외국인 +0.38조 — 이레 만에 순매수 전환</strong>(1주 누적은 여전히 -8.87조). <strong>기관 +1.53조</strong>도 매수했고, <strong>개인은 -3.58조</strong>로 급등장에 차익실현했습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 18일 6,894pt로 전일 +2.66% 급등하며 주간 낙폭을 거의 되돌렸습니다(1주 -0.23%). 외국인이 +0.38조 원으로 이레 만에 매수 전환했고 기관도 +1.53조 원 가세했습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 18일 7,651pt로 전일 +0.17% 상승했습니다. 나스닥 +0.39%, 다우 -0.18%로 혼조세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,019pt로 전일 +1.38% 올랐습니다(1주 +1.57%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.996%(+4.9bp)로 다시 5% 문턱입니다(1개월 +29.0bp). SOFR가 +23bp 급등해 단기 자금시장 경색 신호도 나타났습니다. 한국 국고채 3년은 4.035%(-2.8bp)로 내렸습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,389원(+0.83%)으로 사흘째 올랐습니다(1주 +3.36%). 원화 약세가 빨라지며 1개월 강세 폭도 -1.73%까지 좁혀졌습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $100.30으로 -2.08% 내렸지만 100달러를 지켰습니다. 두바이유는 $118.40(-7.50%) 급락했으나 1개월 +31.13%이며, 브렌트유는 $103.87입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,385.90으로 +0.91% 올라 이틀째 상승입니다. 은은 $66.56(+3.53%)로 더 크게 올랐습니다.'),
]
