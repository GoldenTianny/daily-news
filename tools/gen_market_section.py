...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/e2d8a2c9-daily_20260901.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/7cc3c18f-20260901.html"
PUB = "archive/2026-08-31.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-01.html"      # 출력
BASE = "2026-08-31"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.01"              # 발행일 (업데이트 표기)
GRAD = "901"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,820 — 전일 +0.46% 반등하며 8월을 마감</strong>했습니다(1개월 +3.40%). 다만 <strong>외국인 -0.68조 이틀째 순매도</strong>, 기관 -1.00조로 수급은 무거웠습니다. 코스닥 834(-0.49%), 미국은 다우 -0.63%·S&P500 -0.45%로 하락했습니다.',
 '💡 <strong>원화값은 뉴스 기준 1,360원대 — 13개월 만의 최고치</strong>를 기록했습니다. 다만 이번 자료의 환율 시세는 미갱신이라 표에는 직전 집계치(1,380.5원)가 표시됩니다. <strong>엔화는 160.12엔</strong>으로 160엔대입니다.',
 '💡 <strong>금 4,478.10달러(-2.61%) 급락 조정</strong>입니다 — 그래도 1개월 +10.95%로 상승분은 큽니다. 유가는 <strong>WTI 83.40달러(+1.42%)</strong>, 브렌트유 88.10달러이며, 두바이유(93.20달러)와 은(66.995달러) 시세는 미갱신이라 수준 참고용입니다.',
 '💡 <strong>미국 10년물 4.750%(+2.8bp)</strong> — 금리인상 경계 속에 4.75%까지 올라왔습니다. <strong>국고채 3년 3.838%(+5.0bp), 회사채 4.516%(+4.0bp)</strong>로 국내 금리도 동반 상승했습니다.',
 '💡 <strong>고객예탁금 96.71조, 신용잔고 32.90조</strong> 수준입니다. 이번 자료의 자금 항목은 직전 집계치가 그대로 유지돼(미갱신) 증감보다 수준으로 보시는 편이 안전합니다.',
 '💡 <strong>외국인 -0.68조 — 이틀째 순매도</strong>(1주 누적 -5.09조). 개인 +0.10조, <strong>기관 -1.00조</strong>였습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 8월 31일 6,820pt로 전일 +0.46% 반등하며 8월을 마감했습니다(1개월 +3.40%). 다만 외국인이 -0.68조 원으로 이틀째 순매도를 이어갔습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 8월 31일 7,677pt로 전일 -0.45% 하락했습니다. 다우 -0.63%, 나스닥 -0.34%입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 66,312pt로 전일 -0.14% 보합이었습니다(1개월 +3.03%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.750%(+2.8bp)로 금리인상 경계 속에 올랐습니다. 한국 국고채 3년도 3.838%(+5.0bp)로 동반 상승했습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 뉴스 기준 1,360원대로 13개월 만의 최고 원화 강세를 기록했습니다. 이번 자료의 시세는 미갱신이라 차트 마지막 값은 직전 집계치(1,380.5원)입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $83.40으로 +1.42% 올랐습니다. 브렌트유 $88.10이며, 두바이유($93.20)는 이번 자료에서 미갱신이라 수준 참고용입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,478.10으로 -2.61% 급락 조정입니다(1개월 +10.95%). 은($67.00)은 이번 자료에서 미갱신입니다.'),
]
