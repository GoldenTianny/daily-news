...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/872f310c-daily_20260914.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/c8c14be1-2026-09-14.html"
PUB = "archive/2026-09-11.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-14.html"      # 출력
BASE = "2026-09-11"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.14"              # 발행일 (업데이트 표기)
GRAD = "914"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,910 — 전일 -1.76%로 7,000선을 내줬습니다</strong>(1주 +3.33%). 금리 불안이 AI 모멘텀을 눌렀고, <strong>외국인 -2.60조·기관 -1.44조 동반 매도</strong>를 개인이 +2.38조로 받아냈습니다. <strong>코스닥 821(-1.95%)</strong>, 닛케이도 -1.93%였지만 <strong>미국은 다우 +0.98%·나스닥 +0.96%</strong>로 반등해 방향이 엇갈렸습니다.',
 '💡 <strong>원/달러 1,343.8원(+0.25%)</strong> — 소폭 되돌렸지만 1,340원대 원화 강세 기조는 유지됩니다(3개월 -11.53%). <strong>엔화는 153.55엔(-0.57%)</strong>으로 강세입니다.',
 '💡 <strong>WTI 100.05달러(+4.16%) — 100달러 시대에 들어섰습니다</strong>(1개월 +20.25%). <strong>두바이유 123.66달러(+1.35%)는 1주 +21.34%</strong>, 브렌트유 104.61달러(+3.36%)입니다. 반면 <strong>은은 64.55달러(-4.99%) 급락</strong>, 금도 4,366.20달러(-1.13%)로 밀렸습니다.',
 '💡 <strong>국고채 3년 4.014%(+8.4bp) — 4%선을 넘었습니다</strong>(1개월 +20.6bp). 회사채도 4.688%(+8.8bp)로 뛰었습니다. <strong>미국 10년물 4.975%(+3.1bp)는 1주 +19.1bp·3개월 +51.0bp</strong>로 5% 목전이고, 영국 10년물은 5.345%입니다. 이번 주 13개국 금리 결정이 예정돼 선진국발 연쇄 긴축 부담이 큽니다.',
 '💡 <strong>고객예탁금 102.85조, 신용잔고 31.99조</strong> 수준입니다. 이번 자료의 자금 항목은 직전 집계치가 그대로 유지돼(미갱신) 증감보다 수준으로 보시는 편이 안전합니다.',
 '💡 <strong>외국인 -2.60조 — 이틀 연속 대규모 순매도</strong>(1주 누적 -3.25조). <strong>기관도 -1.44조</strong>로 매도 전환했고, <strong>개인이 +2.38조</strong> 저가 매수에 나섰습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 11일 6,910pt로 전일 -1.76%, 7,000선을 내줬습니다(1주 +3.33%). 금리 불안에 외국인 -2.60조 원·기관 -1.44조 원이 동반 매도했고, 개인이 +2.38조 원으로 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 11일 7,657pt로 전일 +0.86% 반등했습니다. 다우 +0.98%, 나스닥 +0.96%로 한국·일본 하락과 방향이 엇갈렸습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 64,011pt로 전일 -1.93% 하락했습니다(1개월 -4.42%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.975%(+3.1bp)로 5% 목전입니다(1주 +19.1bp, 3개월 +51.0bp). 한국 국고채 3년은 4.014%(+8.4bp)로 4%선을 넘어섰습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,343.8원(+0.25%)으로 소폭 되돌렸습니다. 3개월 기준 -11.53%의 원화 강세 기조는 유지되고 있습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $100.05로 +4.16% 올라 100달러 시대에 들어섰습니다(1개월 +20.25%). 두바이유 $123.66(1주 +21.34%), 브렌트유 $104.61(+3.36%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,366.20으로 -1.13% 하락했습니다(3개월 +6.75%). 은은 $64.55로 -4.99% 급락했습니다.'),
]
