...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/adf87338-daily_20260908.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/84ade351-20260908.html"
PUB = "archive/2026-09-07.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-08.html"      # 출력
BASE = "2026-09-07"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.08"              # 발행일 (업데이트 표기)
GRAD = "908"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,995 — 전일 +4.61% 급등, 7,000 턱밑</strong>까지 왔습니다(1개월 +11.77%). 유동성이 반도체로 쏠리며 삼전닉스가 급등을 이끌었고, <strong>외국인 +2.48조·기관 +2.52조 쌍끌이 매수</strong>에 개인은 -6.65조 대규모 차익실현이었습니다. <strong>닛케이 +2.02%</strong>도 강세였지만 미국은 다우 -0.51%·S&P500 -0.38%로 하락했습니다.',
 '💡 <strong>원화값이 두 달 새 200원 급등</strong>해 국민연금이 환헤지를 중단했다는 소식입니다. 이번 자료의 원/달러 시세는 미갱신이라 표에는 직전 집계치(1,349.5원)가 표시됩니다. <strong>엔화는 156.21엔</strong>으로 보합입니다.',
 '💡 <strong>브렌트유 96.28달러(+0.68%)</strong>로 강세를 이어갔고(1개월 +21.32%), 은은 66.05달러(+2.05%) 반등했습니다. <strong>WTI(91.48달러)·두바이유(101.91달러)·금(4,429.80달러)</strong> 시세는 이번 자료에서 미갱신이라 수준 참고용입니다.',
 '💡 <strong>미국 10년물 4.799%(+1.5bp) — 4.8% 목전</strong>입니다(1개월 +14.1bp). <strong>국고채 3년 3.900%(+1.6bp)</strong>로 1개월 +15.4bp, 회사채 4.575%(+0.8bp)로 국내 금리도 상단을 높여가고 있습니다. 원화 강세와 美 금리인상 우려에 美국채를 담은 서학개미 부담이 커진 상황입니다.',
 '💡 <strong>고객예탁금 97.76조(-4.51조)</strong>로 다시 100조 아래로 내려왔습니다(9/4 집계). 반면 <strong>신용잔고 33.11조(+1,012억)·미수잔고 0.99조(+672억)</strong>는 늘어 급등장에 빚투가 따라붙는 모습입니다. 수익증권은 -6,066억입니다.',
 '💡 <strong>외국인 +2.48조·기관 +2.52조 — 쌍끌이 순매수</strong>(외국인 이틀째, 1주 누적 +0.65조). <strong>개인은 -6.65조</strong>로 급등장에 대규모 차익실현에 나섰습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 7일 6,995pt로 전일 +4.61% 급등하며 7,000 턱밑까지 왔습니다(1개월 +11.77%). 반도체로 유동성이 쏠리며 외국인 +2.48조 원·기관 +2.52조 원 쌍끌이 매수였고, 개인은 -6.65조 원 차익실현이었습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 7일 7,719pt로 전일 -0.38% 하락했습니다. 다우 -0.51%, 나스닥 -0.29%로 한국 급등과 방향이 엇갈렸습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 66,334pt로 전일 +2.02% 급등했습니다(1개월 +1.11%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.799%(+1.5bp)로 4.8% 목전입니다(1개월 +14.1bp). 한국 국고채 3년도 3.900%(+1.6bp)로 1개월 +15.4bp 올라 상단을 높여가고 있습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 두 달 새 200원 가까이 급락(원화 강세)해 국민연금이 환헤지를 중단했습니다. 이번 자료의 시세는 미갱신이라 차트 마지막 값은 직전 집계치(1,349.5원)입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $91.48 수준이며 이번 자료에서 미갱신입니다. 브렌트유는 $96.28(+0.68%)로 강세를 이어갔고(1개월 +21.32%), 두바이유 $101.91은 미갱신 수준 참고용입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,429.80 수준으로 이번 자료에서 미갱신입니다(1개월 +8.17%). 은은 $66.05(+2.05%)로 반등했습니다.'),
]
