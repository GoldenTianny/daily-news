...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/23027b8f-daily_20260917.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/ee97fae8-2026-09-17.html"
PUB = "archive/2026-09-16.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-17.html"      # 출력
BASE = "2026-09-16"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.17"              # 발행일 (업데이트 표기)
GRAD = "917"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,718 — 전일 +1.37%, 닷새 만에 반등</strong>했습니다(1주 -4.73%). 금리가 반락하며 숨통이 트였고 <strong>기관 +1.23조</strong>가 받쳤지만, <strong>외국인은 -1.71조로 닷새째 순매도</strong>(1주 -13.03조)입니다. 코스닥 816(+0.44%), 미국은 나스닥 +0.54%·S&P500 +0.43%, 닛케이 +0.69%로 동반 반등했습니다.',
 '💡 <strong>원/달러 1,363.7원(+1.48%) — 하루 만에 20원 급등</strong>하며 원화 약세로 돌아섰습니다(1주 +1.65%). 유가 급등과 외국인 매도가 겹친 영향으로, 1개월 -3.86%의 강세 폭이 빠르게 줄고 있습니다. <strong>엔화도 155.29엔(+0.60%)</strong>으로 역대급 엔저가 이어집니다.',
 '💡 <strong>WTI 105.83달러(+5.78%) — 1주 +13.76%</strong>로 급등세가 가팔라졌습니다. <strong>두바이유 127.70달러(+3.27%)는 3개월 +61.85%</strong>, 브렌트유 108.75달러(+3.96%)입니다. 반면 <strong>금 4,291.60달러(-1.71%)는 나흘째 하락</strong>, 은 63.24달러(-2.04%)로 귀금속은 약세입니다.',
 '💡 <strong>미국 10년물 4.979%(-1.7bp) — 5% 문턱에서 반락</strong>했습니다. 베선트 재무장관은 금리 급등을 재정적자 탓으로 돌렸습니다. <strong>국고채 3년 4.053%(-3.8bp)</strong>, 회사채 4.716%(-3.4bp), 영국 10년물 5.318%(-6.6bp)로 글로벌 금리가 일제히 숨 고르기에 들어갔습니다. CD 91일물만 3.21%(+4bp)로 올랐습니다.',
 '💡 <strong>고객예탁금 106.82조(-2,381억)</strong> — 1주 +12.89조로 대기자금은 여전히 두텁습니다(9/15 집계). <strong>신용잔고 32.19조(+3,500억)</strong>로 반등 첫날 빚투가 다시 늘었고, 미수잔고 0.99조(-441억)입니다. 수익증권은 -4.52조 감소입니다.',
 '💡 <strong>외국인 -1.71조 — 닷새 연속 순매도</strong>(1주 누적 -13.03조, 1개월 -22.50조). <strong>기관 +1.23조</strong>가 매수로 돌아섰고, <strong>개인은 -1.17조</strong> 반등에 차익실현했습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 16일 6,718pt로 전일 +1.37%, 닷새 만에 반등했습니다(1주 -4.73%). 금리 반락에 기관 +1.23조 원이 받쳤지만 외국인은 -1.71조 원으로 닷새째 순매도입니다(1주 -13.03조). 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 16일 7,618pt로 전일 +0.43% 반등했습니다. 나스닥 +0.54%, 다우 +0.08%로 금리 반락에 회복했습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 63,923pt로 전일 +0.69% 반등했습니다(1개월 -6.97%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.979%(-1.7bp)로 5% 문턱에서 반락했습니다(1주 +14.2bp). 한국 국고채 3년도 4.053%(-3.8bp)로 숨 고르기입니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,363.7원(+1.48%)으로 하루 만에 20원 급등하며 원화 약세로 돌아섰습니다. 1개월 기준 -3.86%로 강세 폭이 빠르게 줄고 있습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $105.83으로 +5.78% 급등했습니다(1주 +13.76%). 두바이유 $127.70(+3.27%)은 3개월 +61.85%, 브렌트유는 $108.75(+3.96%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,291.60으로 -1.71% 내려 나흘째 하락입니다(1개월 -2.03%). 은은 $63.24(-2.04%)입니다.'),
]
