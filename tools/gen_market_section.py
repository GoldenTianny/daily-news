...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/c353bf60-daily_20260910.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/f3f1cf97-20260910.html"
PUB = "archive/2026-09-09.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-10.html"      # 출력
BASE = "2026-09-09"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.10"              # 발행일 (업데이트 표기)
GRAD = "910"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 7,052 — 전일 +1.40%, 33거래일 만에 7,000선을 회복</strong>했습니다(1주 +7.45%, 1개월 +12.67%). 유가 충격에도 <strong>기관 +0.92조</strong>가 받쳤고, 외국인은 -0.17조 소폭 순매도, 개인 -2.54조 차익실현이었습니다. <strong>코스닥도 830(+2.28%)</strong> 강세였지만, 미국은 다우 -0.61%·나스닥 -0.65%로 하락했습니다.',
 '💡 <strong>원/달러 1,341원(-0.37%)</strong> — 1,340원대 초반까지 내려왔습니다(3개월 -12.04%). 원화 강세에 화장품주가 숨 고르기에 들어갔다는 소식입니다. <strong>엔화는 153.61엔</strong>입니다.',
 '💡 <strong>두바이유 109.71달러(+7.65%) — 중동전 확전에 급등, 1개월 +35.86%</strong>입니다. WTI 93.03달러(+1.69%), 브렌트유 97.92달러(+1.70%)도 올랐습니다. 반면 <strong>금은 4,393.90달러(-0.81%)</strong> 조정, 은 66.30달러(+0.38%), 알루미늄 3,325달러(+0.99%)입니다.',
 '💡 <strong>미국 10년물 4.805%(+0.1bp) — 4.8%선에 올라섰습니다</strong>(1개월 +14.7bp). <strong>영국 10년물 5.238%(+6.6bp)</strong>는 1개월 +31bp로 급등세입니다. 국고채 3년 3.910%(+0.9bp), 회사채 4.581%(+0.6bp)로 국내 금리도 소폭 올랐습니다.',
 '💡 <strong>고객예탁금 96.99조(+3.06조) 회복</strong>했습니다(9/8 집계). <strong>신용잔고 32.32조(-4,454억)·미수잔고 0.88조(-2,648억)</strong>로 이틀째 줄어 급등장에서 레버리지는 오히려 정리되는 모습입니다. 수익증권은 -5,102억입니다.',
 '💡 <strong>외국인 -0.17조 — 소폭 순매도 전환</strong>(1주 누적 +3.77조). <strong>기관 +0.92조</strong>가 매수를 이어갔고(1주 +6.43조), <strong>개인은 -2.54조</strong>로 1주 새 -16.97조 대규모 차익실현입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 9일 7,052pt로 전일 +1.40%, 33거래일 만에 7,000선을 회복했습니다(1개월 +12.67%). 유가 충격에도 기관 +0.92조 원이 받쳤고, 개인은 -2.54조 원 차익실현이었습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 9일 7,639pt로 전일 -0.45% 하락했습니다. 다우 -0.61%, 나스닥 -0.65%로 유가 급등 부담에 약세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,143pt로 전일 -0.19% 보합이었습니다(1주 +1.27%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.805%(+0.1bp)로 4.8%선에 올라섰습니다(1개월 +14.7bp). 한국 국고채 3년은 3.910%(+0.9bp)로 1개월 +16.4bp입니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,341원(-0.37%)으로 1,340원대 초반까지 내려왔습니다. 3개월 기준 -12.04%의 원화 강세입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $93.03으로 +1.69% 올랐습니다. 중동전 확전에 두바이유가 $109.71(+7.65%)로 급등해 1개월 +35.86%이며, 브렌트유는 $97.92(+1.70%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,393.90으로 -0.81% 조정받았습니다(1개월 +1.23%). 은은 $66.30(+0.38%)입니다.'),
]
