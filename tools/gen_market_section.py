...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/03acf1a3-daily_20260902.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/a20fe019-20260902.html"
PUB = "archive/2026-09-01.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-02.html"      # 출력
BASE = "2026-09-01"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.02"              # 발행일 (업데이트 표기)
GRAD = "902"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,836 — 전일 +0.23%로 이틀째 상승</strong>했습니다. 다만 <strong>코스닥은 821(-1.56%)</strong>로 밀렸고, <strong>외국인 -0.64조 사흘째 순매도</strong>, 기관 -0.91조로 수급은 여전히 무겁습니다. 미국은 다우 -0.76%·나스닥 -1.02%로 하락했습니다.',
 '💡 <strong>원/달러 1,369.5원(-0.80%) — 1,360원대 진입</strong>, 원화 강세가 가팔라졌습니다(1개월 -5.29%). <strong>엔화는 159.75엔</strong>으로 160엔 아래로 내려왔습니다.',
 '💡 <strong>유가 반등 — 브렌트유 90.49달러(+2.71%)로 90달러 재돌파</strong>, WTI 85.76달러(+2.83%), 두바이유 94.99달러(+1.92%)입니다. 반면 <strong>금은 4,431.10달러(-1.05%) 이틀째 하락</strong>(1주 -4.52%), 은도 66.22달러(-1.16%)로 조정입니다.',
 '💡 <strong>미국 10년물 4.782%(+2.4bp) — 1주 새 +14.3bp</strong>로 상승 압력이 이어집니다. <strong>영국 10년물은 5.224%(+14.7bp) 급등</strong>했고, 일본 30년물도 3%를 넘었습니다. <strong>국고채 3년 3.878%(+4.0bp)</strong>, 회사채 4.549%(+3.3bp)로 국내 금리도 동반 상승했습니다.',
 '💡 <strong>고객예탁금 98.70조(+2.00조) 회복</strong>했습니다(8/28 집계). 신용잔고 32.90조(+42억)는 보합, 미수잔고 0.93조(-930억)로 줄었습니다. 두 달 새 정기예금에 56조가 몰렸다는 기사처럼 대기자금이 예금으로 이동하는 흐름은 유의할 부분입니다.',
 '💡 <strong>외국인 -0.64조 — 사흘째 순매도</strong>(1주 누적 -2.07조). 개인 -0.11조, <strong>기관 -0.91조</strong>로 3주체 모두 매도 우위였습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 1일 6,836pt로 전일 +0.23%, 이틀째 올랐습니다. 다만 외국인이 -0.64조 원으로 사흘째 순매도를 이어가 수급은 무겁습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 1일 7,633pt로 전일 -0.69% 하락했습니다. 나스닥 -1.02%, 다우 -0.76%로 금리 부담에 약세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 66,215pt로 전일 -0.15% 보합이었습니다(1개월 +2.88%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.782%(+2.4bp)로 1주 새 +14.3bp 올랐습니다. 영국 10년물은 5.224%(+14.7bp) 급등했고, 한국 국고채 3년도 3.878%(+4.0bp)로 동반 상승했습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,369.5원(-0.80%)으로 1,360원대에 진입했습니다. 1개월 기준 -5.29%로 원화 강세가 가팔라졌습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $85.76으로 +2.83% 반등했습니다. 브렌트유가 $90.49(+2.71%)로 90달러를 재돌파했고, 두바이유는 $94.99입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,431.10으로 -1.05% 내려 이틀째 하락했습니다(1주 -4.52%). 은은 $66.22(-1.16%)입니다.'),
]
