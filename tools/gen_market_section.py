...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/4ab7f64e-daily_20260922.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/58666a2e-2026-09-22.html"
PUB = "archive/2026-09-21.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-22.html"      # 출력
BASE = "2026-09-21"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.22"              # 발행일 (업데이트 표기)
GRAD = "922"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 7,007 — 전일 +1.65%로 7,000선을 되찾았습니다</strong>(1주 +4.84%). 반도체 수출 호조와 배당 기대가 삼전 관련주 랠리로 이어졌고, <strong>기관 +1.49조</strong>가 매수를 이끌었습니다. 외국인은 -0.09조 소폭 순매도, <strong>개인은 -3.04조</strong> 차익실현입니다. <strong>미국은 나스닥 +2.26%·S&P500 +1.49%</strong>로 동반 급등했습니다.',
 '💡 <strong>원/달러 1,375원(-1.01%) — 나흘 만에 하락 전환</strong>했습니다. 다만 1주 기준으로는 여전히 +1.97%로 원화 약세 흐름이 남아 있습니다. <strong>엔화는 157.34엔(+0.22%)</strong>으로 1주 +1.93% 약세입니다.',
 '💡 <strong>유가와 금·은 시세는 이번 자료에서 미갱신</strong>이라 수준 참고용입니다 — WTI 100.30달러, 두바이유 118.40달러, 브렌트유 103.87달러, 금 4,385.90달러, 은 66.56달러. <strong>알루미늄만 3,285달러(-0.76%)</strong>로 갱신돼 소폭 내렸습니다.',
 '💡 <strong>미국 10년물 4.962%(-3.4bp)</strong> — 5% 문턱에서 다시 물러섰고, <strong>영국 10년물 5.220%(-6.4bp)</strong>도 큰 폭 내렸습니다. 반면 <strong>국고채 3년 4.056%(+2.1bp)</strong>, 회사채 4.724%(+1.8bp)로 국내 금리는 올라 1개월 +20.2bp의 상승세가 이어집니다. SOFR는 3.85%로 1주 +23bp 오른 수준을 유지했습니다.',
 '💡 <strong>고객예탁금 98.33조(+8,435억)</strong>로 소폭 회복했지만 1주 기준 -8.72조입니다(9/18 집계). <strong>신용잔고 32.50조(-1,567억)·미수잔고 0.89조(-1,234억)</strong>가 함께 줄어 레버리지는 정리되는 흐름입니다. 수익증권은 +6.49조 증가입니다.',
 '💡 <strong>외국인 -0.09조 — 소폭 순매도</strong>(1주 누적 -5.51조). <strong>기관 +1.49조</strong>가 1주 +3.77조로 매수 주체 역할을 이어갔고, <strong>개인은 -3.04조</strong>로 이틀 연속 대규모 차익실현입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 21일 7,008pt로 전일 +1.65%, 7,000선을 되찾았습니다(1주 +4.84%). 반도체 수출 호조와 배당 기대에 기관이 +1.49조 원 매수를 이끌었고, 개인은 -3.04조 원 차익실현이었습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 21일 7,765pt로 전일 +1.49% 급등했습니다. 나스닥 +2.26%, 다우 +0.71%로 기술주가 강세를 이끌었습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,019pt 수준이며 이번 자료에서 미갱신이라 수준 참고용입니다(1주 +1.57%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.962%(-3.4bp)로 5% 문턱에서 물러섰습니다. 반면 한국 국고채 3년은 4.056%(+2.1bp)로 올라 1개월 +20.2bp의 상승세가 이어집니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,375원(-1.01%)으로 나흘 만에 하락 전환했습니다. 다만 1주 기준 +1.97%로 원화 약세 흐름은 남아 있습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $100.30 수준이며 이번 자료에서 미갱신입니다. 두바이유 $118.40, 브렌트유 $103.87도 같은 이유로 수준 참고용입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,385.90 수준으로 이번 자료에서 미갱신입니다. 은($66.56)도 미갱신이라 수준 참고용입니다.'),
]
