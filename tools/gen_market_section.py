...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/786b15e5-daily_20260918.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/79447b88-2026-09-18.html"
PUB = "archive/2026-09-17.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-18.html"      # 출력
BASE = "2026-09-17"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.18"              # 발행일 (업데이트 표기)
GRAD = "918"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,715 — 전일 -0.04% 보합</strong>으로 반등 하루 만에 숨을 골랐습니다(1주 -4.53%). 美 기준금리 4% 인상에도 <strong>미국은 나스닥 +1.69%·S&P500 +1.14%</strong>로 불확실성 해소에 강세였고, <strong>코스닥도 822(+0.76%)</strong> 올랐습니다. 다만 <strong>외국인은 -2.59조로 엿새째 순매도</strong>(1개월 -25.09조)이며, 개인 +0.65조·기관 +0.30조가 받아냈습니다.',
 '💡 <strong>원/달러 1,382원(+1.34%) — 이틀 새 33원 급등</strong>해 1,380원대로 올라섰습니다(1주 +2.29%). 한미 금리차가 1%p로 벌어지며 외국인이 코스피에서 피신하는 흐름과 맞물렸고, 1개월 -2.50%로 원화 강세 폭은 대부분 반납됐습니다. <strong>엔화는 155.97엔</strong>입니다.',
 '💡 <strong>두바이유 128.00달러(+1.03%) — 3개월 +74.89%</strong>로 고유가가 더 굳어졌습니다. WTI 102.43달러(+1.03%), 브렌트유 105.83달러(+0.14%)입니다. <strong>금은 4,346.30달러(+0.84%)로 닷새 만에 반등</strong>, 은도 64.29달러(+1.22%)입니다.',
 '💡 <strong>미국 10년물 4.948%(-5.6bp) — 이틀째 하락</strong>하며 5% 문턱에서 멀어졌습니다. 영국 10년물 5.214%(-8.9bp)도 큰 폭 반락했습니다. 반면 <strong>국고채 3년 4.063%(+1.0bp)</strong>, 회사채 4.726%(+1.0bp)로 국내 금리는 소폭 올라, 한은의 11월 금리인상 전망에 무게가 실립니다.',
 '💡 <strong>고객예탁금 99.57조(-5.75조) — 100조선을 내줬습니다</strong>(9/16 집계). 반면 <strong>신용잔고 32.66조(+2,452억)·미수잔고 1.04조(+1,030억)</strong>는 이틀째 늘어, 대기자금은 빠지고 빚투는 붙는 흐름이라 유의할 부분입니다. 수익증권은 +2.97조입니다.',
 '💡 <strong>외국인 -2.59조 — 엿새 연속 순매도</strong>(1주 누적 -11.86조, 1개월 -25.09조). <strong>개인 +0.65조, 기관 +0.30조</strong>가 매수로 대응했습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 17일 6,715pt로 전일 -0.04% 보합, 반등 하루 만에 숨을 골랐습니다(1주 -4.53%). 외국인이 -2.59조 원으로 엿새째 순매도를 이어갔고(1개월 -25.09조), 개인·기관이 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 17일 7,638pt로 전일 +1.14% 상승했습니다. 美 기준금리 4% 인상에도 불확실성 해소에 나스닥 +1.69%, 다우 +0.62%로 강세였습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 64,136pt로 전일 +0.33% 올랐습니다(1개월 -7.34%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.948%(-5.6bp)로 이틀째 하락하며 5% 문턱에서 멀어졌습니다. 한국 국고채 3년은 4.063%(+1.0bp)로 소폭 올라 한은 11월 인상 전망에 무게가 실립니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,382원(+1.34%)으로 이틀 새 33원 급등해 1,380원대에 올라섰습니다. 한미 금리차 확대와 외국인 이탈이 맞물렸고, 1개월 -2.50%로 원화 강세 폭이 대부분 반납됐습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $102.43으로 +1.03% 올랐습니다. 두바이유 $128.00(+1.03%)은 3개월 +74.89%로 고유가가 굳어졌고, 브렌트유는 $105.83입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,346.30으로 +0.84% 올라 닷새 만에 반등했습니다(1주 -1.58%). 은은 $64.29(+1.22%)입니다.'),
]
