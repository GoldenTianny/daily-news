...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/be98ea4d-daily_20260903.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/09bfde3b-20260903.html"
PUB = "archive/2026-09-02.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-03.html"      # 출력
BASE = "2026-09-02"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.03"              # 발행일 (업데이트 표기)
GRAD = "903"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,563 — 전일 -3.99% 급락</strong>했습니다. 글로벌 국채 금리 급등에 따른 금리인상 공포로 <strong>코스닥(-2.10%)·닛케이(-2.85%)도 동반 추락</strong>했습니다. <strong>외국인 -1.85조 나흘째 순매도</strong>, 기관 -2.28조를 개인이 +2.47조로 받아냈습니다. 반면 미국은 S&P500 +0.50%로 반등해 온도차가 컸습니다.',
 '💡 <strong>원/달러 1,375.5원(-0.36%)</strong> — 급락장에도 1,370원대 원화 강세를 유지했습니다(1개월 -4.88%). <strong>엔화는 160.20엔(+0.29%)</strong>으로 40년 만의 최저 수준까지 밀렸습니다.',
 '💡 <strong>유가 급등 — WTI 90.22달러(+8.18%)로 90달러 돌파</strong>, 브렌트유 94.65달러(+7.43%), <strong>두바이유 99.91달러(+7.20%)로 100달러 목전</strong>입니다. 반면 <strong>금은 4,348.00달러(-2.91%) 사흘째 급락</strong>(1주 -6.25%), 은도 64.62달러(-3.55%)로 밀렸습니다.',
 '💡 <strong>국고채 3년 3.930%(+5.2bp) — 1주 새 +11.4bp, 1개월 +17.2bp</strong>로 상승세가 가파릅니다. 회사채도 4.600%(+5.1bp)로 4.6%대에 올라섰습니다. 미국 10년물은 4.793%(-0.3bp), 영국 10년물 5.234%(-2.1bp)로 해외는 숨 고르기였지만 수준 자체가 높습니다.',
 '💡 <strong>고객예탁금 99.70조(+9,995억)</strong>로 100조선 회복을 눈앞에 뒀습니다(8/31 집계). <strong>신용잔고 32.81조(-899억)</strong>로 소폭 줄었고, 미수잔고 0.93조는 보합입니다. 수익증권은 -9,401억 감소입니다.',
 '💡 <strong>외국인 -1.85조 — 나흘째 순매도</strong>(1주 누적 -3.71조, 1개월 -14.50조). <strong>기관도 -2.28조 대규모 매도</strong>였고, <strong>개인이 +2.47조 저가 매수</strong>로 맞섰습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 2일 6,563pt로 전일 -3.99% 급락했습니다. 글로벌 국채 금리 급등에 따른 금리인상 공포로 외국인이 -1.85조 원 나흘째 순매도, 기관도 -2.28조 원 매도였고 개인이 +2.47조 원으로 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 2일 7,670pt로 전일 +0.50% 반등했습니다. 나스닥 +0.38%, 다우 +0.36%로 한국·일본 급락과 온도차가 컸습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 64,326pt로 전일 -2.85% 급락했습니다(1주 -2.92%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.793%(-0.3bp)로 보합이지만 1주 새 +12.9bp 오른 높은 수준입니다. 한국 국고채 3년은 3.930%(+5.2bp)로 1개월 +17.2bp의 가파른 상승세입니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,375.5원(-0.36%)으로 급락장에도 1,370원대 원화 강세를 유지했습니다. 1개월 기준 -4.88%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $90.22로 +8.18% 급등하며 90달러를 돌파했습니다. 브렌트유 $94.65(+7.43%), 두바이유는 $99.91(+7.20%)로 100달러 목전입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,348.00으로 -2.91% 급락, 사흘째 하락입니다(1주 -6.25%). 은은 $64.62(-3.55%)입니다.'),
]
