...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/d38cd04d-daily_20260916.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/856a0c3c-2026-09-16.html"
PUB = "archive/2026-09-15.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-16.html"      # 출력
BASE = "2026-09-15"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.16"              # 발행일 (업데이트 표기)
GRAD = "916"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,627 — 전일 -0.85%로 나흘 연속 하락</strong>했습니다(1주 -4.71%). 미국발 긴축 공포에 조선·방산·전력주가 무너졌고, <strong>외국인 -1.54조(1주 누적 -11.50조)</strong>·기관 -0.80조 매도가 이어졌습니다. 다만 <strong>코스닥은 812(+0.70%)로 반등</strong>했습니다. 미국도 다우 -0.71%·나스닥 -0.78%로 약세입니다.',
 '💡 <strong>원/달러 1,348.5원(+0.35%)</strong> — 이틀째 소폭 되돌렸지만 1개월 -4.93%의 원화 강세 기조는 유지됩니다. 원화 강세로 수입물가는 3개월째 하락했습니다. <strong>엔화는 154.42엔(+0.65%)</strong>입니다.',
 '💡 <strong>두바이유 126.70달러(+2.46%) — 1개월 +43.16%, 3개월 +52.32%</strong>로 고유가가 굳어졌습니다. WTI 101.39달러(+1.34%)도 100달러 위를 지켰고, 브렌트유는 105.68달러(+1.02%)입니다. 반면 <strong>금 4,310.00달러(-1.29%)·은 63.51달러(-1.61%)</strong>는 사흘째 하락했습니다.',
 '💡 <strong>미국 10년물 4.997%(+3.6bp) — 사실상 5% 문턱</strong>입니다(1주 +19.3bp, 3개월 +52.8bp). <strong>국고채 3년 4.091%(+6.6bp)</strong>도 1주 +19.0bp·3개월 +34.7bp로 가파르고, 회사채는 4.750%(+6.0bp)입니다. 2,000조 기업대출의 부실 우려가 함께 커지는 국면입니다.',
 '💡 <strong>고객예탁금 107.06조(-6,013억)</strong>로 소폭 줄었지만 1주 기준 +13.51조로 대기자금은 두텁습니다(9/14 집계). <strong>신용잔고 31.84조(-868억)</strong>는 1주 -1.32조로 계속 감소해 레버리지는 정리되는 흐름입니다. 미수잔고 1.03조(+88억)입니다.',
 '💡 <strong>외국인 -1.54조 — 나흘 연속 순매도</strong>(1주 누적 -11.50조, 1개월 -20.81조). <strong>개인 +0.69조</strong>가 받아냈고, 기관은 -0.80조입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 15일 6,627pt로 전일 -0.85%, 나흘 연속 하락했습니다(1주 -4.71%). 미국발 긴축 공포에 외국인이 -1.54조 원 순매도를 이어갔습니다(1주 누적 -11.50조). 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 15일 7,585pt로 전일 -0.45% 하락했습니다(1주 -1.15%). 나스닥 -0.78%, 다우 -0.71%로 금리 부담에 약세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 63,484pt로 전일 -0.01% 보합이었습니다(1개월 -7.61%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.997%(+3.6bp)로 사실상 5% 문턱입니다(1주 +19.3bp, 3개월 +52.8bp). 한국 국고채 3년도 4.091%(+6.6bp)로 1주 +19.0bp 가파르게 올랐습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,348.5원(+0.35%)으로 이틀째 소폭 되돌렸습니다. 1개월 기준 -4.93%의 원화 강세 기조는 유지되고 있습니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $101.39로 +1.34% 올라 100달러 위를 지켰습니다. 두바이유는 $126.70(+2.46%)로 1개월 +43.16%, 브렌트유는 $105.68(+1.02%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,310.00으로 -1.29% 내려 사흘째 하락입니다(1주 -2.70%). 은은 $63.51(-1.61%)입니다.'),
]
