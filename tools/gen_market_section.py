...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/ca48ab8a-daily_20260915.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/a85f88f4-2026-09-15.html"
PUB = "archive/2026-09-14.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-15.html"      # 출력
BASE = "2026-09-14"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.15"              # 발행일 (업데이트 표기)
GRAD = "915"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,684 — 전일 -3.26% 급락, 사흘 연속 하락</strong>입니다(1주 -4.45%). 중동 3중 봉쇄에 따른 유가 급등과 금리 부담이 겹쳤고, <strong>삼성전자 -4%·하이닉스 -6% 급락</strong>이 지수를 끌어내렸습니다. <strong>외국인 -3.40조·기관 -1.20조 동반 매도</strong>를 개인이 +3.09조로 받아냈습니다. 닛케이도 -0.81%, 미국은 보합권입니다.',
 '💡 <strong>원/달러는 1,343.8원 수준</strong>입니다(1개월 -4.93% 원화 강세 기조). 다만 이번 자료의 환율 시세는 직전 집계치가 그대로 유지돼(미갱신) 수준 참고용입니다. <strong>엔화는 153.42엔</strong>으로 강세를 이어갔습니다.',
 '💡 <strong>유가는 WTI 100.05달러·두바이유 123.66달러·브렌트유 104.61달러 수준</strong>이지만, 이번 자료에서 유가와 금·은 시세는 미갱신이라 수준 참고용입니다. 중동 3중 봉쇄로 실제 유가는 더 뛴 것으로 전해집니다. <strong>알루미늄은 3,274달러(-2.33%)</strong>로 갱신돼 하락했습니다.',
 '💡 <strong>국고채 3년 4.025%(+1.1bp) — 4%선 위에 자리 잡았습니다</strong>(1주 +12.5bp, 1개월 +22.9bp). 회사채 4.690%(+0.2bp), <strong>미국 10년물 4.984%(+0.9bp)는 5% 목전</strong>(1주 +19.2bp), 영국 10년물 5.393%(+4.6bp)입니다. 한은은 전쟁 장기화 시 내년 성장률 2.6% 둔화를 경고했습니다.',
 '💡 <strong>고객예탁금 107.66조(+4.81조) — 1주 새 +9.90조 급증</strong>했습니다(9/11 집계). 급락장에 대기자금이 크게 유입됐습니다. <strong>신용잔고 31.93조(-581억)</strong>는 1주 -1.18조로 줄었고, 미수잔고는 1.02조(+1,516억)입니다.',
 '💡 <strong>외국인 -3.40조 — 사흘 연속 대규모 순매도</strong>(1주 누적 -9.14조, 1개월 -19.23조). <strong>개인이 +3.09조</strong> 저가 매수로 맞섰고, 기관은 -1.20조입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 14일 6,684pt로 전일 -3.26% 급락, 사흘 연속 하락했습니다(1주 -4.45%). 유가 급등과 금리 부담에 반도체가 무너졌고, 외국인 -3.40조 원·기관 -1.20조 원 매도를 개인이 +3.09조 원으로 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 14일 7,638pt로 전일 -0.25% 보합권입니다. 나스닥 -0.02%, 다우 -0.14%로 한국 급락과 온도차가 있었습니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 63,493pt로 전일 -0.81% 하락했습니다(1개월 -7.60%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.984%(+0.9bp)로 5% 목전입니다(1주 +19.2bp). 한국 국고채 3년은 4.025%(+1.1bp)로 4%선 위에 자리 잡았습니다(1개월 +22.9bp).'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,343.8원 수준으로 1개월 -4.93%의 원화 강세 기조입니다. 이번 자료에서는 시세가 미갱신이라 차트 마지막 값은 직전 집계치입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $100.05 수준이며 이번 자료에서 미갱신입니다. 두바이유 $123.66, 브렌트유 $104.61도 같은 이유로 수준 참고용입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,366.20 수준으로 이번 자료에서 미갱신입니다. 은($64.55)도 미갱신이라 수준 참고용입니다.'),
]
