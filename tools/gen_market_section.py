...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/da997583-daily_20260923.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/3233e922-2026-09-23.html"
PUB = "archive/2026-09-22.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-23.html"      # 출력
BASE = "2026-09-22"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.23"              # 발행일 (업데이트 표기)
GRAD = "923"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 7,018 — 전일 +0.15% 강보합으로 7,000선을 지켰습니다</strong>(1주 +5.89%). 메타 호재에 올랐다가 유가에 발목이 잡힌 하루였고, <strong>3주체 모두 소폭 매도</strong>(외국인 -0.05조·개인 -1.42조·기관 -0.23조)였습니다. 코스닥 834(-0.23%), 미국은 나스닥 +0.45%·S&P500 -0.02%로 혼조였습니다.',
 '💡 <strong>원/달러 1,374원(-1.08%) — 이틀 연속 하락</strong>하며 원화가 강세로 돌아섰습니다(1개월 -1.01%). 다만 1주 기준으로는 여전히 +1.89%입니다. <strong>엔화는 157.39엔</strong>으로 보합입니다.',
 '💡 <strong>유가가 급락했습니다 — WTI 95.78달러(-4.51%)로 100달러를 내줬고</strong>, 두바이유 113.69달러(-3.98%), 브렌트유 100.34달러(-3.40%)입니다. 1주 기준으로도 WTI -5.53%, 두바이유 -10.27%로 조정 폭이 큽니다. <strong>금 4,345.80달러(-0.91%)·은 65.83달러(-1.10%)</strong>도 내렸습니다.',
 '💡 <strong>미국 10년물 4.937%(-2.6bp) — 사흘째 하락</strong>해 1주 -5.9bp입니다. <strong>영국 10년물 5.191%(-1.5bp)는 1주 -19.4bp</strong>로 낙폭이 큽니다. <strong>국고채 3년 4.040%(-1.6bp)</strong>, 회사채 4.713%(-1.1bp)로 국내 금리도 1주 -5.1bp 내려 부담을 덜었습니다.',
 '💡 <strong>고객예탁금 98.14조(-1,960억)</strong>로 보합권입니다(9/21 집계). <strong>신용잔고 32.64조(+1,419억)</strong>는 소폭 늘었고, 미수잔고 0.87조(-182억)는 줄었습니다. 은행권 대출여력이 바닥났다는 소식 속에서 증시 자금은 큰 변동 없이 유지되는 모습입니다.',
 '💡 <strong>외국인 -0.05조 — 관망세</strong>(1주 누적 -3.99조). <strong>개인 -1.42조, 기관 -0.23조</strong>로 3주체 모두 소폭 매도 우위였지만, 기관은 1주 기준 +4.38조 순매수를 유지하고 있습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 22일 7,018pt로 전일 +0.15% 강보합, 7,000선을 지켰습니다(1주 +5.89%). 메타 호재에 올랐다가 유가에 발목이 잡혔고 3주체 모두 소폭 매도 우위였습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 22일 7,763pt로 전일 -0.02% 보합이었습니다. 나스닥 +0.45%, 다우 -0.19%로 혼조세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,019pt 수준이며 이번 자료에서도 미갱신이라 수준 참고용입니다.'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.937%(-2.6bp)로 사흘째 하락했습니다(1주 -5.9bp). 한국 국고채 3년도 4.040%(-1.6bp)로 1주 -5.1bp 내려 금리 부담을 덜었습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,374원(-1.08%)으로 이틀 연속 하락하며 원화가 강세로 돌아섰습니다. 1주 기준으로는 여전히 +1.89%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $95.78로 -4.51% 급락해 100달러를 내줬습니다(1주 -5.53%). 두바이유 $113.69(-3.98%), 브렌트유는 $100.34(-3.40%)입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,345.80으로 -0.91% 내렸습니다(1개월 -6.02%). 은은 $65.83(-1.10%)입니다.'),
]
