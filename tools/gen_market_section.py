...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/4092b6da-daily_20260831.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/ab303abd-20260831.html"
PUB = "archive/2026-08-28.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-08-31.html"      # 출력
BASE = "2026-08-28"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.08.31"              # 발행일 (업데이트 표기)
GRAD = "831"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 6,789 — 전일 -1.79%</strong>로 나흘 연속 상승 뒤 쉬어갔습니다. 워시 美 연준 의장 후보의 금리인상 시사 발언이 부담이었고, <strong>외국인이 -1.85조 순매도</strong>로 돌아섰습니다. <strong>코스닥은 838(+0.09%)로 보합</strong>, 닛케이는 66,406(+0.41%)로 올랐습니다. 1개월 기준으로는 코스피 +12.70%로 상승폭이 큽니다.',
 '💡 <strong>원/달러 1,380.5원(-0.40%)</strong> — 원화 강세가 이어졌습니다(1개월 -5.13%). <strong>엔화는 160.07엔(+0.43%)</strong>으로 160엔대에 올라섰습니다.',
 '💡 <strong>두바이유 93.20달러(+7.00%) 급반등</strong>했습니다(1개월 +18.59%). WTI 83.53달러(+1.42%), 브렌트유 89.70달러(+1.26%)도 올랐습니다. 반면 <strong>금은 4,610.20달러(-0.60%)</strong>, 은 66.995달러(-1.46%)로 조정입니다.',
 '💡 <strong>미국 10년물 4.722%(+5.0bp)</strong> — 금리인상 시사 발언에 다시 올랐습니다. <strong>국고채 3년 3.788%(+3.3bp), 회사채 4.476%(+2.8bp)</strong>로 국내 금리도 반등했습니다. CD 91일물은 3.13%로 1주 새 +18bp 뛰었습니다.',
 '💡 <strong>고객예탁금 96.71조(-2.21조) — 1주 새 -8.65조 급감</strong>했습니다(8/27 집계). 반면 <strong>신용잔고 32.90조(+2,335억)</strong>는 계속 늘어 대기자금은 줄고 빚투는 느는 흐름입니다.',
 '💡 <strong>외국인 -1.85조 — 순매도 전환</strong>(1주 누적 -8.74조). <strong>개인 +0.52조</strong>가 받아냈고, 기관은 -0.29조입니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 8월 28일 6,789pt로 전일 -1.79%, 나흘 연속 상승 뒤 쉬어갔습니다. 美 금리인상 시사 발언이 부담으로 작용하며 외국인이 -1.85조 원 순매도로 돌아섰습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 8월 28일 7,712pt로 전일 -0.25% 소폭 하락했습니다. 나스닥 -0.52%, 다우는 보합입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 66,406pt로 전일 +0.41% 올랐습니다(1개월 +6.48%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.722%(+5.0bp)로 금리인상 시사 발언에 다시 올랐습니다. 한국 국고채 3년도 3.788%(+3.3bp)로 반등했습니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,380.5원(-0.40%)으로 원화 강세가 이어졌습니다. 1개월 기준 -5.13%입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $83.53으로 +1.42% 올랐습니다. 두바이유가 $93.20으로 +7.00% 급반등했고(1개월 +18.59%), 브렌트유는 $89.70입니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,610.20으로 -0.60% 조정받았습니다(1개월 +13.15%). 은은 $67.00(-1.46%)입니다.'),
]
