...
# ---------- CONFIG (매일 갱신) ----------
XLSX = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/dea0822d-daily_20260911.xlsx"
DRAFT = "/root/.claude/uploads/405f6ada-29ad-5785-ac2d-569e3af54ca7/f1722d3c-20260911.html"
PUB = "archive/2026-09-10.html"      # 구조 추출용 직전 발행본
OUT = "archive/2026-09-11.html"      # 출력
BASE = "2026-09-10"                  # 종가 기준일 (Keys 시트 최근일자)
PUB_DATE = "2026.09.11"              # 발행일 (업데이트 표기)
GRAD = "911"                          # 차트 gradient id suffix (발행일)
...
# ---------- 팁 6개 (매일 재작성) ----------
TIPS = [
 '💡 <strong>코스피 7,034 — 전일 -0.25%, 유가·금리·동시만기 부담에도 7,000선을 지켰습니다</strong>(1주 +6.91%). <strong>외국인 -3.60조 대규모 순매도</strong>를 기관 +1.82조·개인 +0.12조가 받아냈습니다. <strong>코스닥은 837(+0.79%)</strong>로 올랐고, 미국은 다우·나스닥 -0.65%·S&P500 -0.58%로 1주 -2%대 약세입니다.',
 '💡 <strong>원/달러 1,340.5원(-0.41%)</strong> — 1,340원선까지 내려와 원화 강세가 이어집니다(3개월 -12.07%). <strong>엔화는 154.40엔(+0.56%)</strong>으로 日 추가 금리인상 전망 속 소폭 약세입니다.',
 '💡 <strong>두바이유 122.01달러(+15.77%) 폭등 — 1주 +19.15%, 1개월 +51.10%</strong>입니다. <strong>브렌트유도 101.21달러(+4.34%)로 100달러를 넘었고</strong>, WTI 96.05달러(+5.00%)입니다. 금은 4,416.00달러(-0.31%) 보합, 은 67.94달러(+2.87%), 알루미늄 3,352달러(+1.27%)입니다.',
 '💡 <strong>미국 10년물 4.926%(+8.9bp) — 1주 +16.4bp, 1개월 +22.8bp 급등</strong>해 4.9%대에 올라섰습니다. 영국 10년물 5.303%(+4.2bp)도 상승 중입니다. <strong>국고채 3년 3.930%(+2.0bp)</strong>, 회사채 4.600%(+1.9bp) — 한은이 추가 금리인상을 예고했고 유럽·일본도 인상 국면이라 금리 부담은 이어질 전망입니다.',
 '💡 <strong>고객예탁금 102.85조(+5.86조) — 100조선을 다시 넘었습니다</strong>(9/9 집계). 반면 <strong>신용잔고 31.99조(-3,287억)</strong>는 사흘째 줄어 1주 새 -1.02조, 미수잔고 0.87조(-121억)입니다. 대기자금은 늘고 빚투는 줄어드는 건전한 흐름입니다. 수익증권은 +2.36조입니다.',
 '💡 <strong>외국인 -3.60조 — 대규모 순매도</strong>(1주 누적 +0.24조). <strong>기관 +1.82조</strong>가 1주 +7.52조로 매수 주체 역할을 했고, <strong>개인은 +0.12조</strong>로 매도를 멈췄습니다.',
]
...
CHARTS = [
 chart('kospi', '🇰🇷 KOSPI', pt2, pt0, pt0, pt0,
   '📖 KOSPI가 9월 10일 7,034pt로 전일 -0.25%, 유가·금리·동시만기 부담에도 7,000선을 지켰습니다(1주 +6.91%). 외국인 -3.60조 원 대규모 순매도를 기관 +1.82조 원이 받아냈습니다. 차트 기간(13개월) 누적은 {KOSPI_CHG}입니다.'),
 chart('sp500', '🇺🇸 S&P500', pt2, pt0, pt0, pt0,
   '📖 미국 S&P500은 9월 10일 7,592pt로 전일 -0.58% 하락했습니다(1주 -2.01%). 다우·나스닥도 -0.65%로 금리 급등 부담에 약세입니다.'),
 chart('nikkei', '🇯🇵 NIKKEI 225', pt0, pt0, pt0, pt0,
   '📖 일본 닛케이는 65,271pt로 전일 +0.20% 보합이었습니다(1주 +1.65%).'),
 chart('us10y', '🇺🇸 미국 국채 10년 금리', lambda v: f"{v:.3f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%", lambda v: f"{v:.2f}%",
   '📖 미국 10년물 금리는 4.926%(+8.9bp)로 급등해 4.9%대에 올라섰습니다(1개월 +22.8bp). 한국 국고채 3년도 3.930%(+2.0bp)로 한은 추가 인상 예고 속 상승세입니다.'),
 chart('usdkrw', '💱 원/달러 환율', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원', lambda v: c0(v)+'원',
   '📖 원/달러 환율은 1,340.5원(-0.41%)으로 1,340원선까지 내려왔습니다. 3개월 기준 -12.07%의 원화 강세입니다.'),
 chart('wti', '🛢️ WTI 원유', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.2f}$", lambda v: f"${v:,.2f}",
   '📖 WTI 원유는 배럴당 $96.05로 +5.00% 급등했습니다. 두바이유는 $122.01(+15.77%)로 폭등해 1개월 +51.10%이며, 브렌트유도 $101.21(+4.34%)로 100달러를 넘었습니다.'),
 chart('gold', '🥇 금 (스팟)', lambda v: f"${v:,.2f}", lambda v: f"${v:,.0f}", lambda v: f"{v:,.0f}$", lambda v: f"${v:,.0f}",
   '📖 금은 온스당 $4,416.00으로 -0.31% 보합입니다(1개월 +1.73%). 은은 $67.94(+2.87%)로 올랐습니다.'),
]
